...
func main() {
	...
	srv := server.New(server.Config{
		...
		DeviceLogPath:     envDefault("PUPBOX_DEVICE_LOG_PATH", "data/device-telemetry.jsonl"),
		DeviceLogLimit:    envInt("PUPBOX_DEVICE_LOG_LIMIT", 2880),
		...
		XiaozhiActiveTime: time.Duration(envInt("PUPBOX_XIAOZHI_ACTIVE_SECONDS", 30)) * time.Second,
		...
		XiaozhiSleepGrace: time.Duration(envInt("PUPBOX_XIAOZHI_SLEEP_GRACE_SECONDS", 15)) * time.Second,
	...
	if envBool("PUPBOX_TTS_PREWARM", true) {
		go func() {
			...
			case <-timer.C:
				...
				if limit := envInt("PUPBOX_TTS_PREWARM_LIMIT", 192); limit > 0 && limit < len(replies) {
